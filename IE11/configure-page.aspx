...
(function ($, spg) {
    ...
    $(document).ready(function () {
        ...
        findHostWebUrl(document.location.pathname).then(function (url) {
            ...
            $.when($.getScript(scriptbase + "sp.js")).done(function () {
            ...
            $.when($.getScript(scriptbase + "SP.UI.Controls.js")
        ...
        function listUserCustomAction(siteOrWeb) {
            ...
            p1.then(function (response) {
                ...
                $.each(response.d.results, function (i, result) {
                    $("ul#" + siteOrWeb + "-user-custom-actions").append("<li>" + " [" + result.Location + "] " + (result.Title || result.Name || "") + " ScriptSrc=" + result.ScriptSrc + " Sequence=" + result.Sequence + "</li>");
                });
            });
            return p1;
        }
        return $.when(listUserCustomAction("site"), listUserCustomAction("web"));
    };
    ...
    spg.installUserCustomAction = function (siteOrWeb) {
        ...
        var block;
        if (isCss) {
            block = ["(function(){", "var head1 = document.getElementsByTagName('head')[0];", "var link1 = document.createElement('link');", "link1.type = 'text/css';", "link1.rel = 'stylesheet';", "link1.href = '" + hostsiteurl + "/" + srcurl + "';", srcid ? "link1.id = '" + srcid + "';" : "", "head1.appendChild(link1);", "})();"].join("");
            action.set_scriptBlock(block);
        } else if (srcid) {
            block = ["(function(){", "var head1 = document.getElementsByTagName('head')[0];", "var script1 = document.createElement('script');", "script1.type = 'text/javascript';", "script1.src = '" + hostsiteurl + "/" + srcurl + "';", "script1.id = '" + srcid + "';", "head1.appendChild(script1);", "})();"].join("");
            action.set_scriptBlock(block);
        } else {
